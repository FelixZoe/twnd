...
import cc.felixzoe.entity.Admin;
import cc.felixzoe.exception.*;
import cc.felixzoe.mapper.AdminMapper;
import cc.felixzoe.properties.VisitorProperties;
import cc.felixzoe.service.*;
...
public class AdminServiceImpl implements AdminService {
     ...
     * @param password
     */
    public void sendVerifyCode(String username, String password) {
        // 验证用户是否存在
        Admin admin = adminMapper.getByUsername(username);
        if(admin == null){
            // 账号不存在
            throw new AccountNotFoundException("验证码发送失败，请检查用户名");
        }
        ...
        if(password == null || password.isEmpty()){
            throw new PasswordErrorException("请输入密码");
        }
        try {
            ...
            if(!hashedPassword.equals(admin.getPassword())){
                throw new PasswordErrorException("密码错误，无法获取验证码");
            }
        } catch(PasswordErrorException e) {
            throw e;
        } catch(Exception e) {
            throw new PasswordErrorException("密码验证失败");
        }
        // 检查是否可以发送验证码
