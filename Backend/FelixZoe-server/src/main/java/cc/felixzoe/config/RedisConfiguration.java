...
public class RedisConfiguration {
    ...
    public RedisTemplate<String, Object> redisTemplate(RedisConnectionFactory redisConnectionFactory,
        ...
        log.info("Redis配置初始化完成, 连接工厂: {}", redisConnectionFactory.getClass().getSimpleName());
    ...
    public CacheManager cacheManager(RedisConnectionFactory redisConnectionFactory,
        ...
        RedisCacheConfiguration defaultConfig = RedisCacheConfiguration.defaultCacheConfig()
                ...
                .computePrefixWith(cacheName -> "felixzoe:" + cacheName + ":");

        // 不同缓存空间的TTL策略
        Map<String, RedisCacheConfiguration> cacheConfigurations = new HashMap<>();

        // 静态数据：很少变化，缓存2小时
        cacheConfigurations.put("personalInfo", defaultConfig.entryTtl(Duration.ofHours(2)));
        cacheConfigurations.put("socialMedia", defaultConfig.entryTtl(Duration.ofHours(2)));
        cacheConfigurations.put("skills", defaultConfig.entryTtl(Duration.ofHours(2)));
        cacheConfigurations.put("experiences", defaultConfig.entryTtl(Duration.ofHours(2)));
        cacheConfigurations.put("friendLinks", defaultConfig.entryTtl(Duration.ofHours(2)));
        ...
        // 音乐列表：很少变化，缓存2小时
        cacheConfigurations.put("musicList", defaultConfig.entryTtl(Duration.ofHours(2)));
        ...
        log.info("Redis缓存管理器初始化完成, 配置 {} 个缓存空间", cacheConfigurations.size());
