...
> 在线演示：[blog.felixzoe.cc](https://blog.felixzoe.cc) · [felixzoe.cc](https://felixzoe.cc) · [cv.felixzoe.cc](https://cv.felixzoe.cc)
...
│   ├── FelixZoe-common/         # 公共模块（工具类、常量、异常）
│   ├── FelixZoe-pojo/           # 实体/DTO/VO
│   └── FelixZoe-server/         # 主服务（Controller、Service、Mapper）
│       └── src/main/resources/
│           ├── application.yml.template      # 配置模板
│           ├── application-dev.yml.template   # 开发环境模板
│           ├── application-prod.yml.template  # 生产环境模板
│           ├── database/felixzoe.sql           # 数据库建表脚本
│           └── mapper/                        # MyBatis XML
├── Frontend-Blog/              # 博客前端
...
```bash
git clone https://github.com/FelixZoe/FelixZoe-Website.git
cd FelixZoe
```
...
```bash
# 创建数据库并导入建表脚本
mysql -u root -p -e "CREATE DATABASE FelixZoe DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;"
mysql -u root -p FelixZoe < Backend/FelixZoe-server/src/main/resources/database/felixzoe.sql
```

### 3. 配置后端

```bash
cd Backend/FelixZoe-server/src/main/resources
...
| `felixzoe.jwt.secret-key` | application.yml | JWT 签名密钥（随机字符串即可） |
| `felixzoe.alioss.*` | application.yml | 阿里云 OSS 配置 |
| `spring.mail.username` | application.yml | QQ 邮箱地址 |
| `spring.mail.password` | application.yml | QQ 邮箱授权码 |
| `felixzoe.email.personal` | application.yml | 发件人昵称 |
| `felixzoe.email.from` | application.yml | 发件人邮箱 |
| `felixzoe.visitor.verify-code` | application.yml | 访客验证码 |
| `felixzoe.website.*` | application.yml | 网站标题和 4 个子站地址 |
| `felixzoe.jwt.ttl` | application-dev.yml | JWT 过期时间（毫秒） |
| `felixzoe.datasource.*` | application-dev.yml | 数据库连接信息 |
| `felixzoe.redis.*` | application-dev.yml | Redis 连接信息 |
...
java -jar FelixZoe-server/target/FelixZoe-server-1.0-SNAPSHOT.jar --spring.profiles.active=dev
```
...
cd Backend
mvn clean package -DskipTests
# 产出：FelixZoe-server/target/FelixZoe-server-1.0-SNAPSHOT.jar

# 使用生产环境配置启动
java -jar FelixZoe-server-1.0-SNAPSHOT.jar --spring.profiles.active=prod
```
